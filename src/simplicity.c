...
enum WeatherKey {
  WEATHER_HUMIDITY_KEY = 0x0,         // TUPLE_INT
...
void handle_init(void) {
  ...
  layer_add_child(weather_holder, text_layer_get_layer(rain_layer));

  temp_layer = text_layer_create(GRect(40, 3, 144 - 45, 28));
  ...
  text_layer_set_font(wxtime_layer,
      fonts_get_system_font(FONT_KEY_GOTHIC_14));
  text_layer_set_text_alignment(wxtime_layer, GTextAlignmentRight);
  layer_add_child(weather_holder, text_layer_get_layer(wxtime_layer));
  ...
  Tuplet initial_values[] = {
    TupletCString(WEATHER_HUMIDITY_KEY, ""),
    TupletCString(WEATHER_TEMPERATURE_KEY, "No Weather"),
    TupletInteger(INVERT_COLOR_KEY, persist_read_bool(INVERT_COLOR_KEY)),
    TupletCString(WEATHER_WXTIME_KEY, ""),
    TupletCString(WEATHER_RAIN_KEY, ""),
  };
  ...
  // FIXME testing code
  battery_text_layer = text_layer_create(GRect(0, 168 - 21, 142, 165));
